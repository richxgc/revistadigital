...
class Estadisticas extends CI_Controller {
	...
	public function index(){
		redirect('/admin/estadisticas/articulos');
	}
	...
	public function stats_articles(){
		...
		$data['active'] = 'stats';
		$data['title'] = 'Estadisticas de artículos';
		$data['user'] = $this->user_name;
		//scripts de la vista
		$data['scripts'] = Array(
			'https://www.google.com/jsapi',
			base_url('scripts').'/system/stats/stats_articles.js'
		);
		
		//carga las vistas
		$this->load->view('system/common/header',$data);
		$this->load->view('system/common/navbar');
		$this->load->view('system/stats/stats_articles');
	...
	public function stats_users(){
		...
		$data['menu'] = $this->admin_model->get_menu_by_user();
		$data['active'] = 'stats';
		$data['title'] = 'Estadisticas de usuarios';
		$data['user'] = $this->user_name;
		//scripts de la vista
		$data['scripts'] = Array(
			'https://www.google.com/jsapi',
	...
	public function get_category_articles() {
		...
		$this->response = $this->stats_model->get_category_articles();
		$this->output->set_content_type('application/json')->set_output(json_encode($this->response));
		return;
	}

	public function get_most_read_articles(){
		if($this->input->is_ajax_request() == FALSE){
			redirect('/admin/estadisticas');
			return;
		}
		...
		$this->load->model('stats_model');
		$this->response = $this->stats_model->get_most_read_articles();
		$this->output->set_content_type('application/json')->set_output(json_encode($this->response));
		return;
	}

	public function get_monthly_publications(){
		if($this->input->is_ajax_request() == FALSE){
			redirect('/admin/estadisticas');
			return;
		}
		...
		$this->load->model('stats_model');
		$this->response = $this->stats_model->get_monthly_publications();
		$this->output->set_content_type('application/json')->set_output(json_encode($this->response));
		return;
	}
